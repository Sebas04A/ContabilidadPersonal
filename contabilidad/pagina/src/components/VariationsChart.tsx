...
export function VariationsChart({ className }: Props) {
  ...
  const activeIndex = selectedIndex !== null && selectedIndex < filteredVariations.length 
      ? selectedIndex 
  ...
  if (isError) {
    return (
      <div className={`w-full h-[400px] flex flex-col items-center justify-center text-red-400 ${className}`}>
          ...
          series: [{
              data: values.map((v, idx) => ({
                  ...
                  itemStyle: { 
                      ...
                      shadowColor: idx === activeIndex ? (v >= 0 ? '#22c55e' : '#ef4444') : 'transparent'
                  }
              })),
              type: 'bar',
              barMaxWidth: 20
  ...
  const getDayComponents = (v: any, drivers: TransactionDriver[]) => {
      ...
      const all = [
          { 
              id: 'bank', 
              name: 'Saldo Cuentas', 
              val: sumSource(ComponentType.BANCA),
              snapshot: v.diff_saldo_neto,
              color: 'text-blue-400',
              hex: '#60a5fa',
          ...
          { 
              id: 'invest', 
              name: 'Inversiones', 
              val: sumSource(ComponentType.INVERSION),
              snapshot: v.diff_notion,
          ...
          { 
              id: 'debt', 
              name: 'Deuda (Activo)', 
              val: sumSource(ComponentType.DEUDA),
              snapshot: v.diff_deuda_acumulada,
          ...
          { 
              ...
              val: sumSource(ComponentType.TARJETA),
              snapshot: v.diff_tarjeta,
          ...
          { 
              id: 'fixed', 
              name: 'Pagos Fijos', 
              val: sumSource(ComponentType.PAGOS_FIJO),
              snapshot: v.diff_pagos_fijos,
              color: 'text-gray-400',
              hex: '#94a3b8',
          ...
          { 
              id: 'val', 
              name: 'Valuación', 
              val: sumSource(ComponentType.INTERPOLADOS),
              snapshot: v.diff_interpolados,
              color: 'text-yellow-400',
              hex: '#facc15',
              type: ComponentType.INTERPOLADOS
          },
          // For 'other', we don't have drivers usually. We keep the residual logic?
          // The user wants 'Totals to be sum of details'. 
          // If 'Other' has no details, it should be 0 unless we force it.
          // But 'unexplained_difference' is exactly that: unexplained.
          // So we display it as is, or maybe explicitly as a discrepancy?
          // Let's keep it for now as a catch-all if valuable.
          { 
              id: 'other', 
              name: 'Otro / Inexplicado', 
              val: v.unexplained_difference, 
              snapshot: v.unexplained_difference,
              color: 'text-slate-400',
              hex: '#64748b',
              type: ComponentType.OTROS
          } 
      ];

      return all.map(item => ({
          ...item,
          discrepancy: item.snapshot - item.val
      })).filter(s => Math.abs(s.val) > 0.01 || Math.abs(s.discrepancy) > 0.1);
  };
          ...
          tooltip: {
              ...
              formatter: (params: any) => {
                  ...
                  if (Math.abs(cmp.discrepancy) > 0.1) {
                      html += `<div class="mb-2 text-xs text-yellow-500 font-mono bg-yellow-900/20 p-1 rounded">
                        ⚠ Incongruencia: ${cmp.discrepancy > 0 ? '+' : ''}${cmp.discrepancy.toFixed(2)} vs Real
                      </div>`;
                  }
  ...
  return (
    <div className={`w-full bg-slate-900/60 p-6 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-xl flex flex-col gap-6 ${className}`}>
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-[500px]">
          ...
          <div className="flex-1 h-full min-h-[300px] flex flex-col">
               <div className="flex justify-between items-end mb-2">
                   <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Histórico Diario</h4>
               </div>
                            ...
                            {activeDay && (
                                <>
                                    <div className="text-center mb-6">
                                        <div className="text-white font-bold text-xl tracking-tight">{activeDay.date}</div>
                                        <div className="text-xs text-slate-500 uppercase tracking-widest mt-1">Resumen de Cambios</div>
                                    </div>
                                    ...
                                    <div className="space-y-6">
                                        {dayComponents.map((cmp, i) => {
                                            ...
                                            const relatedTx = dailyDrivers.filter(d => d.source === cmp.type);
                                            
                                            // Sort tx by amount magnitude
                                            relatedTx.sort((a,b) => Math.abs(b.amount) - Math.abs(a.amount));
                                            ...
                                            <div key={i} className="flex flex-col gap-2">
                                                ...
                                                <div className="flex justify-between items-center border-b border-white/5 pb-1">
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-2 h-2 rounded-full" style={{ backgroundColor: cmp.hex }}></div>
                                                        <span className={`text-sm font-medium ${cmp.color} brightness-125`}>{cmp.name}</span>
                                                    </div>
                                                    <span className={`font-mono font-bold text-sm ${cmp.val >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                                        {cmp.val > 0 ? '+' : ''}{cmp.val.toFixed(2)}
                                                    </span>
                                                </div>
                                                        ...
                                                        {relatedTx.map((tx, idx) => (
                                                            <div key={idx} className="flex justify-between items-center text-xs group">
                                                                <div className="flex flex-col flex-1 mr-4">
                                                                    <span className="text-slate-300 truncate w-[180px]" title={tx.description}>
                                                                        {tx.description.replace(/Transferencia Directa/gi, 'T. Directa')}
                                                                    </span>
                                                                    {tx.source && (
                                                                        <span className="text-[10px] text-slate-500 uppercase font-medium">{tx.source}</span>
                                                                    )}
                                                                </div>
                                                ...
                                                {/* Check for residuals */}
                                                {relatedTx.length === 0 && Math.abs(cmp.val) > 1 && (
                                                     <div className="pl-4 text-[10px] text-slate-600 italic">
                                    ...
                                    <div className="mt-8 pt-4 border-t border-dashed border-white/10 flex justify-between items-center bg-slate-900/30 -mx-4 px-4 pb-2">
                                        <div className="flex flex-col">
                                            <span className="text-slate-400 text-xs uppercase">Total del Día</span>
                                            <span className="text-[10px] text-slate-600">(Calculado)</span>
                                        </div>
                                        <span className={`font-mono font-bold text-2xl ${calculatedTotal >= 0 ? 'text-green-400' : 'text-red-400'}`}>
