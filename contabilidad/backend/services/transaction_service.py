...
- Loading enrichment data (horas_tarjeta.csv, read-only)
...
# Enrichment: exact time-of-day recovered from the bank's "Notificación de Consumos"
# emails. Generated by scripts/enriquecer_horas_tarjeta.py. Optional by design: if
# the file is missing, everything below degrades to HORA = None.
HORAS_PATH = os.path.join(PATH_DATA, 'sistema', 'enriquecimiento', 'horas_tarjeta.csv')
HORAS_COLUMNS = ['source_id', 'HORA']
...
def load_horas() -> pd.DataFrame:
    """
    Load the time-of-day enrichment CSV (source_id -> HORA).

    Read-only and strictly optional: this service never writes to it. The file is
    produced by scripts/enriquecer_horas_tarjeta.py from the bank's consumption
    emails. If it is absent or unreadable we return an empty frame so callers
    simply see HORA = None rather than failing.
    """
    if not os.path.exists(HORAS_PATH):
        return pd.DataFrame(columns=HORAS_COLUMNS)

    try:
        df = pd.read_csv(HORAS_PATH, usecols=lambda c: c in HORAS_COLUMNS)
    except Exception as e:
        logger.warning("Could not read %s: %s", HORAS_PATH, e)
        return pd.DataFrame(columns=HORAS_COLUMNS)

    for col in HORAS_COLUMNS:
        if col not in df.columns:
            logger.warning("Column %s missing in %s, ignoring enrichment", col, HORAS_PATH)
            return pd.DataFrame(columns=HORAS_COLUMNS)

    # One time per transaction. Defensive: a duplicated source_id would otherwise
    # multiply rows in the join below and silently inflate every total.
    df = df.drop_duplicates(subset='source_id', keep='first')
    return df[HORAS_COLUMNS]
...
def _hora_de_fecha(fechas: pd.Series) -> pd.Series:
    """
    Time already carried by FECHA itself.

    The bank statement states the time of every account movement
    ('2026-7-13, 4:21 PM'), and the reading pipeline keeps it inside FECHA. So
    for BANCA the time needs no enrichment at all — it just has to be read out.

    Midnight is treated as "no time": it is what rows without one collapse to
    (all card rows, and account rows before 2024-09 that come from the legacy
    CSV). A genuine 00:00 movement is indistinguishable from a missing one here,
    and reporting no time is the safer of the two errors.
    """
    horas = fechas.dt.strftime('%H:%M')
    return horas.where(fechas.dt.time != time(0, 0), '').fillna('')


def _attach_horas(merged: pd.DataFrame) -> pd.DataFrame:
    """
    Resolve the 'HORA' column ('HH:MM', or '' when unknown) from its two origins.

    Two sources, one column:
      · BANCA   -> the time is already in FECHA (the statement provides it).
      · TARJETA -> the statement has no time; it was recovered from the bank's
                   consumption emails into horas_tarjeta.csv, joined by source_id.

    Purely additive: adds only 'HORA' and touches nothing else, so no amount,
    balance or aggregation can change. Rows with no time from either origin keep
    '' rather than a fabricated one.
    """
    hora_fecha = _hora_de_fecha(merged['FECHA'])
    horas = load_horas()

    if horas.empty:
        merged['HORA'] = hora_fecha
        return merged
    ...
    drop_cols = [c for c in merged.columns if c.endswith('_hora')]
    drop_cols.append('source_id')
    merged = merged.drop(columns=[c for c in drop_cols if c in merged.columns], errors='ignore')

    # Missing time is the empty string, matching how every other text column in
    # this service reports "no value" (see sanitize_for_json's str_cols).
    #
    # It must NOT be left as NaN or None: under pandas 3 this column is inferred
    # as the `str` dtype, whose missing value is a float NaN, and any later
    # .copy() re-introduces it — which makes the whole endpoint fail to
    # serialize with "Out of range float values are not JSON compliant".
    # fillna('') is stable across that inference; None is not.
    merged['HORA'] = merged['HORA'].fillna('').astype(str)

    # The enrichment wins where it exists (card rows); FECHA covers the rest
    # (account rows). They never compete: card FECHAs are always midnight.
    # .values because hora_fecha is positional and the merge preserved order.
    merged['HORA'] = merged['HORA'].where(merged['HORA'] != '', hora_fecha.values)
    return merged
