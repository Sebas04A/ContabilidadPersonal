...
from contabilidad.backend.services.bank_parser import cuenta as LecturaCuenta
...
from contabilidad.models import SavedAccountChangesData, SavedChangesData, EnhancedJSONEncoder
import json

from contabilidad.backend.storage.snapshots import verificacion_cambios
...
def crear_data_cuenta_guardado(df_cuentas,path_carpeta,nueva_cuenta_config:FileProcessingConfig):
    nueva_cuenta=LecturaCuenta.read_new_account(nueva_cuenta_config)
...
def obtener_data_guardado_cuenta(df_cuenta,nueva_cuenta_config:FileProcessingConfig)->DataCambiosGuardadoCuenta:
    df_nuevo = LecturaCuenta.read_new_account(nueva_cuenta_config)
    ...
    cambios =""
    return SavedAccountChangesData(fecha_inicio=fecha_inicio,fecha_fin=fecha_fin,saldo_inicio=saldo_inicio,saldo_fin=saldo_fin,path_nuevos_datos=nueva_cuenta_config.path,cambios=cambios)
...
def guardar_prueba_nuevos_datos(df_completo,df_cuentas,nueva_cuenta_config:FileProcessingConfig,nombre_carpeta=None):
    ...
    datos_guardado = SavedChangesData(
        fecha=datetime.now(),
