-- Vuelta atrás de 20260912180000_pago_manual_antes_que_el_cruce.sql.
--
-- Guion de emergencia, no una migración numerada. Restaura EXACTAMENTE las funciones que
-- había en producción el 2026-09-12 antes de aplicarla: se bajaron con
-- pg_get_functiondef() de la base real, no se reconstruyeron desde los ficheros.
--
-- Ejecutar ENTERO (va en una transacción). No toca datos: los pagos que se hayan
-- registrado con la versión nueva quedan como están, y siguen siendo válidos para la
-- versión vieja (son pagos y detalles normales).
...
CREATE OR REPLACE FUNCTION public._ec_lado(p_pov text, p_es_mi_deuda boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.estado_cuenta(p_deudor_id uuid, p_pov text DEFAULT 'owner'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.aplicar_cruce(p_deudor_id uuid, p_fecha date DEFAULT CURRENT_DATE, p_idem_key uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
...
GRANT EXECUTE ON FUNCTION _ec_lado(TEXT, BOOLEAN)          TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION estado_cuenta(UUID, TEXT)          TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION aplicar_cruce(UUID, DATE, UUID)    TO anon, authenticated, service_role;
